...
void AP_SerialManager::init()
{
    ...
    for (uint8_t i = 1; i < SERIALMANAGER_NUM_PORTS; i++)
    {
        ...
        if (uart != nullptr)
        {
            ...
            case SerialProtocol_HFC:
            {
                uart->begin(
                    AP_SERIALMANAGER_HYDROGEN_BAUD,
                    AP_SERIALMANAGER_HYDROGEN_BUFSIZE_RX,
                    AP_SERIALMANAGER_HYDROGEN_BUFSIZE_TX);
                uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);

                hfc.bind(uart);

                hal.console->printf("HFC: bound to SERIAL%u (baud=%u rx=%u tx=%u)\n",
                                    (unsigned)i,
                                    (unsigned)AP_SERIALMANAGER_HYDROGEN_BAUD,
                                    (unsigned)AP_SERIALMANAGER_HYDROGEN_BUFSIZE_RX,
                                    (unsigned)AP_SERIALMANAGER_HYDROGEN_BUFSIZE_TX);
                break;
            }
