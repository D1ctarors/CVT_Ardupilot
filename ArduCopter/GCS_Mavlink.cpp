...
bool GCS_MAVLINK_Copter::try_send_message(enum ap_message id)
{
    ...
    case MSG_HFC_STATUS:
    {
        CHECK_PAYLOAD_SIZE(HYDROGEN_PLANT);

        extern AP_HFC hfc;

        if (!hfc.healthy() || (AP_HAL::millis() - hfc.last_update_ms() > 1000))
        {
            return false;
        }

        const auto &d = hfc.data();

        mavlink_msg_hydrogen_plant_send(
            chan,
            d.identifikator,
            d.current,
            d.battery_voltage,
...
MAV_RESULT GCS_MAVLINK_Copter::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    ...
    case MAV_CMD_DO_HYDROGEN_CONTROL:
    {
        const uint8_t id = (uint8_t)packet.param1;
        const bool enable = packet.param2 > 0.5f;

        hfc.request_switch(id, enable);
